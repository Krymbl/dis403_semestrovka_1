...
Нужно использовать шаблонизатор Freemarker или JSP . В коде шаблонов нельзя напрямую вызывать Java-код. Должна быть развитая система шаблонов (свои теги, разделение видов на несколько частей). 
Сайт должен иметь аутентификацию, авторизацию, регистрацию.
Сайт должен хранить данные в СУБД: PostgreSQL , MySQL, MSSQL, MariaDB. 
...
В работе нужно показать использование WebFilter, WebListener. Второй можно использовать для инициализации карты используемых сервисов (DAO/Repository, сервисы бизнес-логики).
Необходимо применить Javascript. Обдуманное использование JavaScript, AJAX не обязательно, но положительно повлияет на оценку.
